...
Current Shelf Tools: hierarchy,priming,padding,jointrename,Ltsv,Lrsv,Lsv, Unlock
						Freezetransforms,del H, CP,Ik, IKRP,IKSC,CVs,circle,sqcv,
						cube,arrow,cluster,PCMO,PCNO,OCMO,OCNO,*CMO,*CNO,PV
						(PC->, OC-> and *C-> are dividers not actual tools)
...
parentConstraint -mo -weight 1;
	#no eco all, first tested 
pm.parentConstraint('','')	
weight(w), skipTranslate(st),targetList(tl), weightAliasList(wal),offset(o)
...
orientConstraint -offset 0 0 0 -weight 1;
	#no eco all, first tested 
pm.orientConstraint('','')
targetList(tl), weightAliasList(wal),weight(w), skipTranslate(st),offset(mo)
...
pointConstraint -offset 0 0 0 -weight 1;
	#no eco all, first tested 
pm.pointConstraint('','')
targetList(tl), weightAliasList(wal), offset(mo)
...
less flags, doesnt have one for offset
'''
...
addAttr(ln='Float', keyable=True)
ongname (ln), attributeType (at), enumerated(e),keyable(k)	
...
addAttr(ln='Separator', at='enum', en='--------', keyable=True)
...
setAttr -e-keyable true;
	...
	#no ecoall, copy straight from atribute editor
	
pm.addAttr(ln='index_curl', keyable=True)
pm.addAttr(ln='middle_curl', keyable=True)
pm.addAttr(ln='pinky_curl', keyable=True)
